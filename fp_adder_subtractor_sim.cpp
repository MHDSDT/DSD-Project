// Run with verilator --top FP_Adder --cc --exe --build fp_adder_subtractor*

#include "obj_dir/VFP_Adder.h"
#include "verilated.h"
#include <iostream>
#include <random>
#include <limits>
...
void test_number(VFP_Adder& module, float a, float b) {
    constexpr float epsilon = 1e-4;
    // Set inputs
    module.a = extract_float_bits(a);
    module.b = extract_float_bits(b);
    // Add
    module.add_sub_not = 1;
    module.eval();
    float expected = a + b;
    ...
    if (std::abs(expected - got) > epsilon && !(std::isnan(got) && std::isnan(expected)) && std::abs((int32_t)module.result - (int32_t)extract_float_bits(expected)) > 1)
        ...
        "; diffrence of these numbers are: " << std::abs(expected - got) << std::endl;
    // Sub
    module.add_sub_not = 0;
    module.eval();
    expected = a - b;
    got = float_from_bits(module.result);
    if (std::abs(expected - got) > epsilon && !(std::isnan(got) && std::isnan(expected)) && std::abs((int32_t)module.result - (int32_t)extract_float_bits(expected)) > 1)
                std::cout << "Invalid result on " << a << " - " << b <<
        ": Expected " << expected << " but got " << got <<
        "; diffrence of these numbers are: " << std::abs(expected - got) << std::endl;
...
int main(int argc, char** argv, char** env) {
    ...
    VFP_Adder top(&context);
    // Hardcoded tests
    test_number(top, float_from_bits(1119388147), float_from_bits(1137940003));
